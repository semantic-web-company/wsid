...
class EntityCoOccurrences:
    def __init__(self,
        ...
        self._graph: ig.Graph or None = None
        self._co_dict: Dict[str, Dict[str, float]] or None = None
    ...
    def pr(self, with_entity=False) -> Dict[str, float]:
        ...
        assert all(pr.values()), list(pr.values())[:10]
        if not with_entity:
    ...
    def get_cos_around_entity(self, neigh_divider: int = 20) -> List[str]:
        ...
        module_logger.debug(f'Number of 1st order coocs: {len(entity_row.data)}')
        ...
        module_logger.debug('Neighbors limit = ' + str(round(neighbors_th)))
        ...
        if len(important_cos_inds) >= 10000:
            important_cos_inds = entity_cos_inds
    ...
    def get_co_graph_dict(
        ...
        def normalize_vals(mx):
            ...
            # denom_v = mx.max(axis=0).todense().A1  # TODO: NEW!!
            # denom_v = mx.mean(axis=0).A1  # TODO: NEW!!
        ...
        entity_vv_ind = vs_tokens.index(self.entity_str)
        entity_row = cos_vv.getrow(entity_vv_ind).toarray()

        co_score_th = cos_vv.sum()/cos_vv.count_nonzero() * co_average_score_th
        cos_vv = cos_vv.multiply(cos_vv > co_score_th).tolil()
        cos_vv[entity_vv_ind, :] = entity_row
        cos_vv[:, entity_vv_ind] = entity_row.T
        cos_vv = cos_vv.tocsr()
        ...
        module_logger.debug(f'edgelist done, size = {len(edgelist)}')
        ...
        module_logger.info(f'Graph done in {co_co_time - start:0.3f}s. '
                           f'Number of nodes: {len(graph.vs)}, '
...
def get_hubs(entity_str: str,
    ...
    def _get_hubs(scoring_func, broaders, th_hub=th_hub):
        ...
        while True:
            ...
            if score > th_hub:
                ...
                assert e_cos.entity_str not in hub_neighbors, hub_neighbors
                hub_involvement = {
                    n: sum(e_cos.co_func(n, z) for z in hub_neighbors - {n})
                        for n in {z
                                  for hub_neighbor in hub_neighbors
                                  for z in e_cos.co_dict[hub_neighbor]}
                        if n != entity_str}
                cluster = dict()
                for x in set(hub_involvement):
                    involvement = hub_involvement[x]
                    assert involvement <= 1, (x, involvement)
                    cluster[x] = e_cos.pr[x] * involvement * len(e_cos.pr) / len(hub_neighbors)#* left_nodes[x]
                    left_nodes[x] -= involvement
                    if left_nodes[x] < 0:
                        left_nodes[x] = 0
                clusters.append(cluster)
                hubs.append((br_hub, initital_score, broaders)) #score
                if broaders:
                    break
    ...
    def get_hyperlex_scoring_func(br_g:Set[str]) -> Callable[[str], float]:
        ...
        def pr_co_score(x: str, br_g: Set[str]) -> float:
            ...
            # val = e_cos.co_func(x, e_cos.entity_str)
            # if br_g:
            #     val += sum(e_cos.co_func(x, br) for br in br_g) / len(br_g)
            val = sum(e_cos.co_func(x, br) for br in br_g_e) / len(br_g_e)
            val *= e_cos.pr[x]
            return val
    ...
    if broader_groups:
        ...
        for broaders in broader_groups:
            ...
            if not br_G:
                module_logger.warning(f'None of broaders {broaders} was found '
                                      f'in the co-occurrence graph.')
            else:
                scoring_func = get_hyperlex_scoring_func(br_g=br_G)
                br_hubs, br_clusters = _get_hubs(scoring_func,
                                                 broaders=broaders)
                hubs += br_hubs
                clusters += br_clusters

    scoring_func = get_hyperlex_scoring_func(br_g=set())
    other_hubs, other_clusters = _get_hubs(scoring_func, broaders=[])
    hubs += other_hubs
